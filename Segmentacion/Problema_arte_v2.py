...
def imshow(img, title=None, color_img=False, show=True):
    plt.figure()
    if color_img:
        plt.imshow(img)
    else:
...
f6_color = cv2.applyColorMap(np.uint8(255/N*f6), cv2.COLORMAP_JET)
imshow(f6_color, title=f'Filtrado de elementos pequeños - Elementos detectados: {N}')
...
# Contornos
contours, hierarchy = cv2.findContours(ff, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE) 
...
for ii in range(1,num_labels):
    ...
    pp=-1
    rel=-1
    if rho < 20:  # --> Son circulos o cuadrados
        ...
        rel = area/box_area
        # --- Graficas ---------------------------------------------------
        # img = cv2.merge((obj8.copy(), obj8.copy(), obj8.copy()))
        # cv2.drawContours(img,[box],0,(0,0,255),1)
        # cv2.putText(img, f'{rel:5.2f}', (centroids[ii][0].astype(int), centroids[ii][1].astype(int)), cv2.FONT_HERSHEY_SIMPLEX, 0.30, (255,0,0), 0)
        # plt.figure()
        # plt.imshow(img), plt.xticks([]), plt.yticks([])
        # # plt.scatter(box[:,0], box[:,1], c='r')
        # plt.scatter(box[0,0], box[0,1], c='r')
        # plt.scatter(box[1,0], box[1,1], c='g')
        # plt.scatter(box[2,0], box[2,1], c='b')
        # plt.scatter(box[3,0], box[3,1], c='c')
        # plt.title(f'{ii:02d} - L1={L1:5.2f}/L2={L2:5.2f} - Barea={box_area:5.2f} - area={area:5.2f} - rel={rel:5.2f}')
        # plt.show()
        # ----------------------------------------------------------------
        if rel < 0.6:
    ...
    X.append(dict(box=box, centroid=centroids[ii], rho=rho, pp=pp, rel=rel))
